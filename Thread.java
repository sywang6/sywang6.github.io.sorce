 ...
 *一个线程实例就是程序中的一个执行线程(Thread类时java对线程的抽象)
 ...
 * 高优先级的线程比低优先级的线程更受偏爱得到允许(但不是说高优先级的线程就一定比低优先级的先允许)
 * Each thread may or may not also be marked as a daemon.
 * 每个线程可能或者不可能被标记为守护线程
 * When code running in some thread creates a new <code>Thread</code> object, the new
 * thread has its priority initially set equal to the priority of the
 * creating thread, and is a daemon thread if and only if the
 * creating thread is a daemon.
 * 一个正在运行的线程,创建一个新的线程,新创建的线程优先级默认被设置为与创建他的线程的优先级一样.守护线程仅能创建守护线程
 * <p>
 *
 * When a Java Virtual Machine starts up, there is usually a single
 * non-daemon thread (which typically calls the method named
 * <code>main</code> of some designated class).
 *当一个jvm启动,通常有一个非守护的线程(这个线程典型调用指定的class的main方法)
 * The Java Virtual Machine continues to execute threads until either of the following
 * occurs:
 *JVM持续的执行线程,直到下面的任何一项发生:
 * <ul>
 * <li>The <code>exit</code> method of class <code>Runtime</code> has been
 *     called and the security manager has permitted the exit operation
 *     to take place.
 *类Runtime的exit方法被调用且安全管理允许退出操作发生
 * <li>All threads that are not daemon threads have died, either by
 *     returning from the call to the <code>run</code> method or by
 *     throwing an exception that propagates beyond the <code>run</code>
 *     method.
 * </ul>
 * 所有非守护线程都已死亡,或者从run方法已经返回,或者run方法抛出异常
 * <p>
 ...
 * <code>Thread</code>.
 *一种方法是继承Thread类,子类重写Thread类的run方法
 ...
 * </pre></blockquote>
 * <p>
 * //这是作者给我们举的一个怎样创建线程的例子-大素数的计算(继承Thread类)
 * The other way to create a thread is to declare a class that
 * implements the <code>Runnable</code> interface. That class then
 * implements the <code>run</code> method.An instance of the class can
 * then be allocated, passed as an argument when creating
 * <code>Thread</code>, and started.
 * 另一种创建线程的方法实现Runnable类,实现run方法.分配实例,作为创建线程的一个参数传递过去,然后在启动线程
 *
 ...
 * </pre></blockquote>
 * <p>
 * //还是上面大素数计算的例子,不过这次作者使用的是实现Runnable的接口,并作为一个参数传递给线程的方式实现的
 ...
 *线程创建的的时候没有指定名称,会自动为其生成一个名称.
 * <p>
 * Unless otherwise noted, passing a {@code null} argument to a constructor
 * or method in this class will cause a {@link NullPointerException} to be
 * thrown.
 *除非另有说明,传递一个空的参数到构造器或者其他方法,将会导致NullPointerException异常抛出
 * @author  unascribed
 * @see     Runnable
...
class Thread implements Runnable {
     ...
     * @see        #start()
     * @see        ThreadDeath
     * @see        ThreadGroup#uncaughtException(java.lang.Thread,Throwable)
     * @see        SecurityManager#checkAccess(java.lang.Thread)
    ...
    private native static StackTraceElement[][] dumpThreads(java.lang.Thread[] threads);
    private native static java.lang.Thread[] getThreads();
